...
threshold = 0.05 
sigma     = 1 
filename  = os.path.join(sys.path[0], "grid.jpg")

I_rgb      = plt.imread(filename)
I_rgb      = I_rgb / 255.0
I_gray     = cm.rgb_to_gray(I_rgb)
I_blur     = cm.gaussian(I_gray, sigma)
Ix, Iy, Im = cm.central_difference(I_blur)
x,y,theta  = cm.extract_edges(Ix, Iy, Im, threshold)

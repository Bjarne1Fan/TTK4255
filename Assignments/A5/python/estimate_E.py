...
def prosac(
  ...
  for i in range(S):
    ...
    if num_inliers >= max_num_inliers:
      ...
      E = estimated_E
    # residuals = epipolar_distance.epipolar_distance(F=F, uv1=uv1, uv2=uv2)
    # normed_residuals = np.linalg.norm(residuals, axis=0) / np.sqrt(2)

    # # Check number of points being inliers
    # num_inliers = np.sum(normed_residuals <= distance_threshold)

    # if num_inliers >= max_num_inliers:
    #   print("Better sample found at iteration number: {}".format(i))
    #   max_num_inliers = num_inliers
    #   inlier_set = np.where(normed_residuals <= distance_threshold)[0]
    #   E = estimated_E
